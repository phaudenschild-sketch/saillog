...
Am Boot ausführen, um IP/Port von Orca Core, B&G-Plotter & Co. zu finden:
    ...
    python -m saillog.discover 192.168.4.1        # TCP-Ports an einem Host scannen
    python -m saillog.discover 192.168.4.1 --full # mehr Ports probieren
    python -m saillog.discover --udp              # auf UDP-Broadcasts lauschen
...
def scan_tcp(host: str, ports: List[int]) -> None:
    ...
    for port in ports:
        ...
        if result:
            print(f"      → In SailLog: Host={host}  Port={port}  Protokoll=tcp")
    if not found:
...
def scan_udp(ports: List[int]) -> None:
    ...
    for port in ports:
        ...
        print(f"      {_format_types(result)}")
        if result:
            print(f"      → In SailLog: Port={port}  Protokoll=udp")
    if not found:
...
def gofree_source_hints(device: Dict) -> List[str]:
    """Liefert saillog-Quellzeilen (TCP host:port) für die NMEA-Dienste."""
    ip = device.get("ip")
    hints: List[str] = []
...
def scan_gofree(seconds: float = 8.0, iface: Optional[str] = None,
    ...
    for d in devices:
        ...
        for hint in gofree_source_hints(d):
            print(f"      → in SailLog als Quelle eintragen:  {hint}")
...
def main(argv=None) -> int:
    ...
    if args.host and args.sweep:
        ...
        print("\nFertig. Die passende Zeile oben in SailLog eintragen und 'Verbinden'.")
        return 0
